package coreJavaPractice.stringStuff;
...
/** Created by Moji on 21-Oct-17. mojtaba.nasehzadeh@gmail.com */
public class StringUtil {
  ...
  public static Map<String, Long> findStringCharsFrequencyJava8(String s) {
    ...
    Map<String, Long> wordFreq =
        stream.collect(Collectors.groupingBy(String::toString, Collectors.counting()));
    return wordFreq;
  }
  ...
  public static int findCharOccurrence(String s, Character c) {
    ...
    List<Character> listOfChars =
        inputStr.chars().mapToObj(ch -> (char) ch).collect(Collectors.toList());
  ...
  /**
   * Finds 1st non repated Character of given String implemented by LinkedHashMap
   *
   * @param s
   * @return
   */
  public static Character findFistNonRepeartedChar(String s) {
    ...
    for (Map.Entry<Character, Integer> c : map.entrySet()) {
      if (c.getValue() == 1) result = c.getKey();
      break;
    }
    return result;
  }
  ...
  /**
   * Anagram Army & Mary The key is : 1) Arrays.sort() 2) Arrays.equals()
   *
   * @param s1
   * @param s2
   */
  ...
  /**
   * horoofe seda dar point: vowels.indexOf(s.charAt(i)) == -1
   *
   * @param s
   * @return
   */
   ...
   * @throws IOException
   */
  private static void linePhraseCounterInFile(String inputFilePath, String outputFilePath)
      throws IOException {
